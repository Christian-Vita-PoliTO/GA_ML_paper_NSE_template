...
transient analyses, are performed on a
...
not respond in the same way to the displacement of a boundary e.g.: a change that
improves the multiplication factor may worsen the flux distribution. A
A measurement made on the available data quantifies how abrupt the
resulting surface is. Among the structures available in this work, those differing by
...
appreciably, $0.9905$ and $1.49\%$, a difference smaller than the spread between
random initialisations. The physical quantity, and not the core configuration,
is therefore the dimension along which the fifteen outputs actually need
separate parameters, which is consistent with the three configurations differing
only in the position of the control rods. The configuration branch was retained
in the production model, since it costs $3\%$ more parameters and does no harm,
but the gain attributable to it is not measurable.
...
The next subsection makes this quantitative.
...
\begin{figure}[htbp]
  ...
  one obtained from the reference solver, for every distinct structure.}
  \label{fig:search}
\end{figure}
...
solved with the reference solver in the loop. They are shown in
