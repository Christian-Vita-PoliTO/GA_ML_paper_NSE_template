...
the analysis of Sec.~\ref{sec:reference} shows how such an agreement is reached:
...
\begin{figure}[htbp]
  ...
  quantities plotted against each other, one point per band and family, with a
  rank correlation of $+0.70$. The error of the metamodel follows the local
  steepness of the target and not the magnitude of the quantity, which is why
  the flux, steep where its values are large, behaves in the opposite way with
  respect to the other four families.}
...
values. No quantity discussed in Sec.~\ref{sec:results} is a prediction of the
metamodel. This arrangement is
...
Section~\ref{sec:howmany} makes this quantitative.
...
what is being asked of the sample.

The first concerns the optimum, and here the sample is more than sufficient. By
re-drawing the hundred outcomes in random order many times, the best structure
found after twenty searches is on average within $0.5\%$ of the value reached
after a hundred, and after thirty searches within $0.2\%$. The optimum itself
was produced by three of the hundred searches, which gives the premise of this
work its quantitative form: a campaign of three tribes has a probability of
$9\%$ of finding it, one of ten tribes $26\%$, one of twenty $46\%$ and one of
fifty $78\%$. The best structure is therefore not something a small campaign
misses by a little; it is something a small campaign is unlikely to see at all.

The second concerns the regularities of Sec.~\ref{sec:patterns}, and here too
...
frequency of a boundary and the importance surrounding it is already $+0.54$
with ten searches, and that going to a hundred narrows its ninetieth-percentile
interval from $[+0.43,+0.60]$ to $[+0.49,+0.57]$ without moving the estimate.
The reason is that the correlation is computed over the 29 internal boundaries
of the fine mesh, and that number does not grow with the number of searches.
More searches stabilise the frequency attached to each boundary; only a finer
starting mesh would add degrees of freedom to the test. The separation into two
families is equally settled: the silhouette coefficient of the Hamming distance
is $0.514$ for two clusters and falls to $0.391$ for three, the third cluster
being a single structure.

The third question is the one the sample does not answer. Treating the searches
as draws from a population of reachable structures, 54 of the 67 were seen once
and only 5 twice, which puts the Good-Turing coverage at $46\%$ and the
Chao1 estimate of the reachable population at about 300 structures, of which
this campaign has seen roughly one fifth. New structures were still appearing at
a rate of $0.55$ per search at the hundredth. A complete census of the good
structures is therefore out of reach, and no claim of that kind is made here.
The consequence is visible in the least robust of the results above: resampling
the 67 structures reproduces the seven-boundary consensus skeleton exactly in
$72\%$ of the replicas and the set of five never-selected boundaries in $43\%$,
because both are defined by a threshold that a boundary near $90$ or $10\%$
crosses in one direction or the other depending on the sample. The frequencies
themselves are stable; their classification into three groups is a convenience
of presentation.
...
Sec.~\ref{sec:limit}: since the best structures owe their quality to a
...
characterised in Sec.~\ref{sec:limit}, appearing here in operation rather than
in diagnosis: the structures proposed by the searches are precisely those whose
...
performed on the same reactor. The analyses of Secs.~\ref{sec:patterns}
and~\ref{sec:results}, which required one hundred searches and thirteen
constrained optimizations, would not have been attempted otherwise.
